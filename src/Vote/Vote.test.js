...
describe('Vote', () => {
  ...
  it('should display all submit button when everyone has voted', () => {
    const { getByText, getByRole, getAllByRole } = render(<MemoryRouter><Vote judges={judges} topic1={topic1} topic2={topic2}/></MemoryRouter>)
    const voteButtonTeam1 = getAllByRole('button', { name: 'Team: Pizza' })
    const vote1 = getByText('Judge Leonardo Votes')
    const vote2 = getByText('Judge Raphael Votes')
    const vote3 = getByText('Judge Michelangelo Votes')
    ...
    expect(vote2).toBeInTheDocument()
    expect(vote3).toBeInTheDocument()
    fireEvent.click(voteButtonTeam1[0])
    fireEvent.click(voteButtonTeam1[1])
    fireEvent.click(voteButtonTeam1[2])
